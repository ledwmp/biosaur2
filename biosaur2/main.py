...
def process_file(args):
    ...
    for scan_range in args['scan_range'].split(','):
        logger.info('Processing scan range: %s', scan_range if scan_range != '' else 'not defined')
        args['scan_low'], args['scan_high'] = map(lambda x: float(x), scan_range.split('-')) if '-' in scan_range else (None, None)
